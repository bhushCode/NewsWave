...
lateinit var progressBar: ProgressBar
   ...
   val view  = inflater.inflate(R.layout.fragment_sports,container,false)
        sportsReView = view.findViewById(R.id.sports_recView)
        progressBar = view.findViewById(R.id.sports_progressBar)
        shimmer = view.findViewById(R.id.shimmer_layout_sports)
        shimmer.startShimmer()
        //    progressBar.visibility = View.VISIBLE
        adapter = NewsAdapter()

        val newsApi = RetrofitHelper().getInstance().create(NewsApi::class.java)
        val repository = NewsRepository(newsApi,"sports",requireContext())
